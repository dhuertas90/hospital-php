...
class TurnosController {
    ...
    public function getTurnosDisponibles($fecha){
        $turnos     =   ModelTurnos::getInstance()->turnosExistentes();
        $mensaje    =   '';
		
        foreach ($turnos as $turno) {
			
            if ($turno['dia'] == $fecha['dia'] && $turno['mes'] == $fecha['mes'] && $turno['anio'] == $fecha['año']){
				
               $mensaje .=	$turno['hora'] . ':' . $turno['minutos'] . ' | ';
            }
        }
    ...
    public function postReservar($datos){
        $id_turno   =   ModelTurnos::getInstance()->getIdTurno($datos);
        ModelTurnos::getInstance()->reservarTurno($datos['documento'], $id_turno['id']);
